...
if strcmp(task,'forward')
    ...
    if numColors == 1
        ...
        %         numFilters = size(kernel,1); filter_batch_size = 4;
        ...
        for b = 1 : filter_batch_num
            % these two paramtners relates to number of filters
            filterPerThread = 4; 
            blockDim_x = 4; 
            % these two parameters relates to input batch size
            imagePerThread = 1; 
            blockDim_y = 32; 
            kConv.ThreadBlockSize = [blockDim_x, blockDim_y];
            if ~my_thread
                kConv.GridSize = [numModulesX * numModulesY * numModulesZ * filter_batch_size / (filterPerThread * blockDim_x), ceil(numImages/(blockDim_y * imagePerThread))]; % filterPerThread, BlockSize.Y
            else
                kConv.GridSize = [ceil(numModulesX * numModulesY * numModulesZ * filter_batch_size / (filterPerThread * blockDim_x)), ceil(numImages/(blockDim_y * imagePerThread))]; % filterPerThread, BlockSize.Y
            end
        ...
        if ~my_thread
            ...
            kConv.GridSize = [ceil(numModulesX * numModulesY * numModulesZ * numFilters / (filterPerThread * 4)), ceil(numImages/(32 * imagePerThread))]; % filterPerThread, BlockSize.Y
        end
